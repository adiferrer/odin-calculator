...
digitButtons.forEach(digit => digit.addEventListener('click', () => {
    ...
    if (currentOperator == '') previousValue += d;
        ...
        if (previousValue != '' && fullExpression.indexOf('=') < 0) {
...
operatorButtons.forEach(operator => operator.addEventListener('click', () => {
    ...
    if (fullExpression.length == 0 && previousValue != '') fullExpression.push(Number(previousValue));
    else if (fullExpression.lastIndexOf('=') < 0) {
        fullExpression.push(Number(currentValue));
        total = operate(Number(previousValue), Number(currentValue), currentOperator);
        previousValue = total;
        currentValue = '';
